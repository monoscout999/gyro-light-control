# PROMPT: Implementar websocket_handler.py - Gestor de Comunicación WebSocket

## CONTEXTO DEL PROYECTO

Estás implementando el manejador de WebSocket para un sistema de control de luces móviles. Este módulo gestiona las conexiones WebSocket entre el servidor Python y los clientes (mobile y frontend), con buffering para compensar latencia de red.

---

## TU TAREA

Crear el archivo `websocket_handler.py` que gestione:
- Conexiones/desconexiones de clientes WebSocket
- Recepción de datos del sensor (alpha, beta, gamma)
- Buffer de latencia (smoothing de datos)
- Broadcast de estados actualizados
...
WebSocketHandler
    ...
    │ Interpolación
    ↓
Server (math_engine + venue + fixtures)
    ↓
WebSocketHandler
    ↓ Broadcast
    │ {sensor, pointer, fixtures}
    ↓
...
## CLASE PRINCIPAL: WebSocketHandler
...
- [ ] LatencyBuffer implementado completamente
- [ ] Interpolación funciona correctamente
- [ ] Alpha wraparound manejado (359° → 1°)
- [ ] WebSocketHandler con conexiones set
- [ ] Métodos async correctos (connect, broadcast, etc.)
- [ ] Buffer integrado en handler
- [ ] Stats retorna info útil
- [ ] Todos los tests pasan (8/8)
- [ ] Logging configurado
- [ ] Manejo de errores en send/receive
...
1. **Tests limitados**:
   - Tests completos de WebSocket requieren servidor corriendo
   - Estos tests cubren la lógica de buffer e inicialización
   - Tests de integración vendrán en server.py

2. **Interpolación**:
   - Suaviza movimiento jerky
   - Predice ligeramente hacia adelante (factor 1.5 max)
   - Crítico para buena UX

3. **Alpha wraparound**:
   - 359° → 1° debe interpolar por 0°, no por 180°
   - Importante para giroscopio (compass)

4. **Async/await**:
   - connect, send, receive son async
   - broadcast es async (envía a múltiples clientes)

---

## TIEMPO ESTIMADO

**20-30 minutos** (más complejo que venue/fixture por async)

---

**ENTREGA**: Archivo `websocket_handler.py` completo con 8 tests pasando.
